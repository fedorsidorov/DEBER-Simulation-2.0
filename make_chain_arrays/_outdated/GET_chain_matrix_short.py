...
particles_matrix = np.zeros((mv.n_Z, mv.n_XY, mv.n_x, mv.n_y, mv.n_z, mv.n_part_max, 3))*np.nan
pos_matrix = np.zeros((mv.n_Z, mv.n_XY, mv.n_x, mv.n_y, mv.n_z))

chain_inv_matrix = np.zeros((mv.n_chains_short, 6000, 7))*np.nan
...
#    for n_mon in range(1):
        ...
        if Z >= mv.n_Z or XY >= mv.n_XY or x >= mv.n_x or y >= mv.n_y or z >= mv.n_z:
            ...
            chain_inv_matrix[n_chain_short, n_mon, :-1] = np.nan
            chain_inv_matrix[n_chain_short, n_mon, -1] = mon_type
            
            continue
        
        pos = pos_matrix[Z, XY, x, y, z].astype(int)
        
        particles_matrix[Z, XY, x, y, z, pos, :] = n_chain_short, n_mon, mon_type
        pos_matrix[Z, XY, x, y, z] += 1
        
        chain_inv_matrix[n_chain_short, n_mon] = Z, XY, x, y, z, pos, mon_type
        
    n_chain_short += 1
        
#%%
np.save('Sharma/MATRIX_particles_short.npy', particles_matrix)
