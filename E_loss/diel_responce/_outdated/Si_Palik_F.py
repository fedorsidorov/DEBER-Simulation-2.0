...
#%%
sun_diel = np.loadtxt('curves/Chan_diel_Si_1000.txt')
...
#%%
S = np.zeros(len(E_arr))
u = np.zeros(len(E_arr))


for i in range(len(E_arr)):
    
    inds = np.where(E_arr <= E_arr[i]/2)
    
    S[i] = np.trapz(diff_IMFP_inv_arr[i, inds] * E_arr[inds]*mc.eV, x=E_arr[inds]*mc.eV)
    u[i] = np.trapz(diff_IMFP_inv_arr[i, inds]*mc.eV, x=E_arr[inds]*mc.eV)


#%%
plt.loglog(E_arr, S / mc.eV / 1e+2)
