...
xx = xx - shifts[n_dose]
yy = yy + height - yy.max()
plt.plot(xx, (yy * 1000 + offset[n_dose]) / 1000, label='ЭКСП')
...
plt.plot(xx, (func(xx, *popt) * 1000 + offset[n_dose]) / 1000, '--', label='МОД')
...
plt.savefig('final_' + str(n_dose) + '_RUS.tiff', bbox_inches='tight', dpi=500)
