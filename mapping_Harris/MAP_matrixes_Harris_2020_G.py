...
ss_matrix = np.load(os.path.join(mc.sim_folder,
        'e-matrix_Harris', 'sweep', bond, 'Harris_ss_' + bond + '_' + str(w) + '.npy'
        ))


total_E_loss = np.sum(np.load(os.path.join(mc.sim_folder,
        'e-matrix_Harris', 'sweep', bond, 'Harris_dE_' + bond + '_' + str(w) + '.npy'
        )))
...
#%%
print('roughly, G =', np.sum(ss_matrix) / total_E_loss * 100)
    ...
    for i in range(int(n_events)):
        ...
        if len(monomer_positions) == 0:
            ...
            if xi+1 < resist_shape[0]:
                ss_matrix[xi+1, yi, zi] += n_events
                break
            
            elif yi+1 < resist_shape[1]:
                ss_matrix[xi, yi+1, zi] += n_events
                break
            
            elif zi+1 < resist_shape[2]:
                ss_matrix[xi, yi, zi+1] += n_events
                break
...
#%%
#np.save('lens_final_' + bond + '_' + str(w) + '.npy', chain_lens_final)
...
#chain_lens_final = np.load('lens_final_' + bond + '_' + str(w) + '.npy')
...
plt.title(bond + '_' + str(w) + ', G = ' + str(np.round(Gs*100) / 100))
...
#plt.savefig(bond + '_' + str(w) + '.png', dpi=300)
