...
def get_collision_ind(layer_ind, E_ind):
    ...
    return get_MC_ind(ma.processes_U[layer_ind][E_ind, :])
...
def get_dE_E2nd_On_O2nd(layer_ind, proc_ind, now_E, E_ind, O_prev):
    ...
    elif (layer_ind == 0 and proc_ind not in [0, 4, 5]) or\
         (layer_ind == 1 and proc_ind in [1, 2, 3, 4]): ## ionization
        
        return get_ion_dE_E2nd_On_O2nd(layer_ind, proc_ind, now_E, E_ind, O_prev)
    ...
    if proc_ind == 4: ## PMMA phonons
        ...
        E_prime = now_E - dE
        B = (now_E + E_prime + 2*np.sqrt(now_E*E_prime)) /\
            (now_E + E_prime - 2*np.sqrt(now_E*E_prime))
        u5 = rnd.random()
        
        cos_theta = (now_E + E_prime)/(2*np.sqrt(now_E*E_prime)) * (1 - B**u5) + B**u5
        theta = np.arccos(cos_theta)
    ...
    else: ## PMMA polarons (proc_ind == 5)
        return now_E, 0, O_prev*0, O_prev*0
