...
u = np.hstack((np.zeros(np.shape(u[:, :1])), u, np.zeros(np.shape(u[:, :1]))))
u = np.vstack((np.zeros(np.shape(u[:1, :])), u, np.zeros(np.shape(u[:1, :]))))
...
#    u[1:-1, 1:-2] += D * dt / d_coord**2 * (
#        un[2:, 1:-2] - 2 * un[1:-1, 1:-2] + un[:-2, 1:-2] +
#        un[1:-1, 2:-1] - 2 * un[1:-1, 1:-2] + un[1:-1, :-3] )
    
    u[1:-1, 1:-1] += D * dt / d_coord**2 * (
...
surf = ax.plot_surface(X, Z, u[1:-1, 1:-1].transpose(), rstride=1,\
