...
#%%
resist_matrix = np.load('MATRIX_resist_2um_NEW2.npy')

chain_tables_folder = 'Chain_tables_EXP_2um_NEW2/'
...
for i in range(N_chains_total):
    ...
    if len(now_chain_table) == 1:
        rewrite_mon_type(resist_matrix, now_chain_table, 0, free_mon)
...
#            go = False
        ...
        while now_zipped < zip_len:
            ...
            rewrite_mon_type(resist_matrix, chain_table, n_mon, free_mon)
            now_zipped += 1
            ...
            if n_next_mon >= 0 and n_next_mon < now_len-1 and\
                chain_table[n_next_mon, mon_type_ind] != free_mon:
                ...
                now_monomer_positions =\
                    np.where(resist_matrix[now_xi, now_yi, now_zi, :,\
                                           mon_type_ind] - 3 < 0)[0]
