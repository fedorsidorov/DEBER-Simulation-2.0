...
#%% Gryzinski
def get_Gr_tau_single(E, Ui, WW=mc.EE):
    
    Gr_tau_single = np.zeros(len(WW))
    ...
    Gr_tau_single[inds] = np.pi * mc.k_el**2 * mc.e**4 / np.power(DE * mc.eV, 3) * Ui/E *\
        np.power(E / (E + Ui), 3/2) * np.power((1 - DE/E), Ui / (Ui + DE)) *\
        (DE/Ui * (1 - Ui/E) + 4/3 * np.log(2.7 + np.sqrt((E - DE) / Ui))) ## m^2 / J
    
#    Gr_tau_single[inds] = tau * (100)**2 * mc.eV ## cm^2 / eV
...
def get_Gr_tau(EE, Ui, WW=mc.EE):
    ...
    for i in range(len(EE)):
        Gr_tau[i, :] = get_Gr_tau(EE[i], Ui, WW)
